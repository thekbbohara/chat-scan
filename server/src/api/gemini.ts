...
import { SYSTEM } from "../config/system_instruction.js";

const { GEMINI_API_KEY } = process.env;
console.log({ GEMINI_API_KEY });
if (!GEMINI_API_KEY) console.log("Undefined GEMINI_API_KEY");
...
export async function filterComments(
  ...
  if (!GEMINI_API_KEY) return { comments: null };
  const res = await model.generateContent([
    `titleOfTheStream: Exploring the Latest Web Development Trends`,
    JSON.stringify({ selectedComments, newComments }),
  ]);
  // console.log("res", res.response.text());
  try {
    const { comments: selectedComments }: { comments: { id: string }[] } =
      JSON.parse(res.response.text());
    // console.log({ selectedComments });
    // console.log({ botResponses });
    const _filteredComments: Comment[] = selectedComments
      .map(({ id }) => newComments?.find((comment) => comment.id == id))
      .filter((comment) => comment !== null) as Comment[];
    // console.log({ _filteredComments });
    return { comments: _filteredComments };
  } catch {
